...
export class Database {
  ...
  bullet_read(filter = 'active') {
    try {
      switch (filter) {
        case 'active':
          return this._sql.all`SELECT id, title, content, image_url, image_path, is_active, version FROM bullet WHERE is_active <> 0 ORDER BY id`;
        case 'archive':
          return this._sql.all`SELECT id, title, content, image_url, image_path, is_active, version FROM bullet WHERE is_active = 0 ORDER BY id`;
        case 'all':
          return this._sql.all`SELECT id, title, content, image_url, image_path, is_active, version FROM bullet ORDER BY id`;
        default:
    ...
    try {
      return this._sql.all`
        SELECT 
          ...
          bullet.is_active,
          bullet.version,
          COALESCE(bullet.image_url, bullet.image_path, bullet_patch.image_url, bullet_patch.image_path) AS image_path,
          json_group_array(json_object(
            'id', bullet_patch.id,
            'title', bullet_patch.title,
            'content', bullet_patch.content,
            'is_active', bullet_patch.is_active,
            'image_path', COALESCE(bullet_patch.image_url, bullet_patch.image_path),
            'changeType', CASE 
   ...
   */
  patch_bullet_create(title, content, imagePath = null) {
    try {
      const res = this._validateModification(
        this._sql.run`
          INSERT INTO bullet_patch (title, content, image_path)
          VALUES (${title}, ${content}, ${imagePath})`
      );
      return res.lastInsertRowid;
    } catch (err) {
      this._handleError(err, { method: 'patch_bullet_create', details: { title, content, imagePath } });
    }
  }

  /**
   * Create a patch for updating an existing bullet (content, image, or status)
   * @param {number} bullet_id - ID of bullet to change
   * @param {string|null} content - Optional new content
   * @param {string|null} imagePath - Optional image path
   * @param {boolean|null} is_active - Optional active status change
   * @return {number} ID of the created patch
   */
  patch_bullet_update(bullet_id, title = null, content = null, imagePath = null, is_active = null) {
    try {
      const res = this._validateModification(
        this._sql.run`
          INSERT INTO bullet_patch (bullet_id, bullet_version, title, content, image_path, is_active)
          VALUES (
            ${bullet_id},
      ...
      this._handleError(err, {
        method: 'patch_bullet_update',
        details: { bullet_id, title, content, imagePath, is_active }
      });
    }
  }
    ...
    try {
      ...
      try {
        ...
        let res = this._sql.run`
          INSERT INTO bullet (title, content, is_active, version, image_path)
          SELECT title, content, COALESCE(is_active, 1), 1, image_path
          FROM bullet_patch
          WHERE id = ${patch_id}
            AND status = 'P'
        ...
        if (!res.changes) {
          ...
          res = this._sql.run`
            UPDATE bullet SET
              ...
              image_path = COALESCE(bullet_patch.image_path, bullet.image_path)
            FROM bullet_patch
