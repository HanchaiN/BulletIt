...
export class Database {
  ...
  /**
   * Read method for bullets with basic filter support
   * @param {string} filter - 'active' (default), 'archive', or 'all'
   * @returns {Array} List of bullets matching the filter
   * @deprecated Prefer `Database.bullet_search`
   */
  bullet_read(filter = 'active') {
    try {
      switch (filter) {
        case 'active':
          return this._sql.all`SELECT * FROM bullet JOIN bullet_active ON bullet.id = bullet_active.id WHERE bullet_active.is_active = TRUE ORDER BY id`;
        case 'archive':
          return this._sql.all`SELECT * FROM bullet JOIN bullet_active ON bullet.id = bullet_active.id WHERE bullet_active.is_active = FALSE ORDER BY id`;
        case 'all':
   ...
   * @param {Object} filter
   * @param {string} filter.searchterm Search term
   * @param {boolean} filter.active Select active
   * @param {boolean} filter.archive Select archive
   * @returns {Array} List of bullets matching the filter
   */
  bullet_search({ searchterm='', active=true, archive=false }) {
    try {
      if (active && archive) {
        return searchterm
          ? this._sql.all`SELECT bullet.*, rank FROM bullet
            JOIN bullet_fts AS f ON bullet.id = f.rowid
            WHERE bullet_fts MATCH ${searchterm}
            ORDER BY rank, bullet.id`
          : this._sql.all`SELECT bullet.*, 0 AS rank FROM bullet
            ORDER BY rank, bullet.id`;
      }
      if (active) {
        return searchterm
          ? this._sql.all`SELECT bullet.*, rank FROM bullet
            JOIN bullet_fts AS f ON bullet.id = f.rowid
            JOIN bullet_active ON bullet.id = bullet_active.id
            WHERE bullet_active.is_active <> 0 AND bullet_fts MATCH ${searchterm}
            ORDER BY rank, bullet.id`
          : this._sql.all`SELECT bullet.*, 0 AS rank FROM bullet
            JOIN bullet_active ON bullet.id = bullet_active.id
            WHERE bullet_active.is_active <> 0
            ORDER BY rank, bullet.id`;
      }
      if (archive) {
        return searchterm
      ...
      this._handleError(err, { method: 'bullet_search', details: { searchterm, active, archive } });
    }
  }
   ...
   */
  unsafe__bulletin_create(content) {
    try {
   ...
   */
  unsafe__bulletin_update_active(id, is_active) {
    try {
      const res = this._validateModification(
   ...
   */
  unsafe__bulletin_update_content(id, content) {
    try {
      const res = this._validateModification(
   ...
   */
  unsafe__bulletin_update_full(id, content, is_active) {
    try {
      const res = this._validateModification(
        this._sql.run`
          UPDATE bullet SET 
   ...
   * @returns {Array} List of pending patches with computed changeType
   */
  patch_bullet_read_pending_group() {
    try {
      return this._sql.all`
        SELECT 
          bullet.id,
          bullet.title AS title,
          COALESCE(bullet_patch.content, bullet.content) AS content,
          bullet.is_active,
          bullet.version,
          bullet.image_uri,
          bullet.expire_at,
          ...
          json_group_array(json_object(
            'id', bullet_patch.id,
            'title', bullet_patch.title,
            'content', bullet_patch.content,
            'is_active', bullet_patch.is_active,
            'image_uri', bullet_patch.image_uri,
            'expire_at', bullet_patch.expire_at,
            'active_at', bullet_patch.active_at,
            'changeType', CASE 
              WHEN bullet_patch.content IS NOT NULL AND bullet_patch.is_active IS NOT NULL THEN 'mixed'
              WHEN bullet_patch.content IS NOT NULL THEN 'content'
              WHEN bullet_patch.is_active IS NOT NULL THEN 'archive'
              ELSE 'unknown'
            END
          )) AS patches
        FROM bullet_patch
        LEFT JOIN bullet ON bullet_patch.bullet_id = bullet.id
        LEFT JOIN bullet_active ON bullet.id = bullet_active.id
        WHERE bullet_patch.status = 'P'
        GROUP BY (CASE WHEN bullet.id IS NOT NULL THEN bullet.id ELSE bullet_patch.id END)
      `.map(row => ({
        id: row.id,
        title: row.title,
        content: row.content,
  ...
  /**
   * Fetch a single patch by ID
   * @param {number} id - Patch ID
   * @returns {Object|null} Patch object or null if not found
   */
  patch_bullet_read_by_id(id) {
    try {
      return this._sql.get`SELECT * FROM bullet_patch WHERE id = ${id}` ?? null;
    } catch (err) {
      this._handleError(err, { method: 'patch_bullet_read_by_id', details: { id } });
    }
  }

  /**
   * Create a new patch for a new bullet
   * @param {Object} patch - New values
   * @param {string} patch.title
   * @param {string} patch.content
   * @param {string | null} patch.image_uri
   * @param {boolean | null} patch.is_active
   * @param {string | number | null} patch.expire_at
   * @param {string | number | null} patch.active_at
   * @return {number} ID of the created patch
   */
  patch_bullet_create_create(patch) {
    try {
      const res = this._validateModification(
        this._sql.run`
          INSERT INTO bullet_patch (title, content, image_uri, is_active, expire_at, active_at)
          VALUES (
            ${patch.title},
            ${patch.content},
            ${patch.image_uri},
            ${patch.is_active ?? null === null ? null : (patch.is_active ? 1 : 0)},
            UNIXEPOCH(${patch.expire_at}, 'auto'),
            UNIXEPOCH(${patch.active_at}, 'auto')
          )
        `
      );
      return res.lastInsertRowid;
    } catch (err) {
      this._handleError(err, { method: 'patch_bullet_create_create', details: { patch } });
    }
  }

  /**
   * Create a patch for changes
   * @param {number} bullet_id - ID of bullet to change
   * @param {Object} patch - New values
   * @param {string | null} patch.title
   * @param {string | null} patch.content
   * @param {string | null} patch.image_uri
   * @param {boolean | null} patch.is_active
   * @param {string | number | null} patch.expire_at
   * @param {string | number | null} patch.active_at
   * @return {number} ID of the created patch
   */
  patch_bullet_create_update(bullet_id, patch) {
    try {
      const res = this._validateModification(
        this._sql.run`
          INSERT INTO bullet_patch (bullet_id, bullet_version, title, content, image_uri, is_active, expire_at, active_at)
          VALUES (
            ${bullet_id},
      ...
      this._handleError(err, { method: 'patch_bullet_create_update', details: { bullet_id, patch } });
    }
  }
    ...
    try {
      ...
      try {
        ...
        let res = this._sql.run`
          INSERT INTO bullet (id, title, content, image_uri, is_active, expire_at, active_at, version)
          SELECT bullet_id, title, content, image_uri, COALESCE(is_active, 1), expire_at, active_at, 1 
          FROM bullet_patch
          WHERE id = ${patch_id}
            AND status = 'P'
        ...
        if (!res.changes) {
          ...
          res = this._sql.run`
            UPDATE bullet SET
              version = version + 1,
              title = COALESCE(bullet_patch.title, bullet.title),
              content = COALESCE(bullet_patch.content, bullet.content),
              is_active = COALESCE(bullet_patch.is_active, bullet.is_active),
              image_uri = COALESCE(bullet_patch.image_uri, bullet.image_uri),
              expire_at = COALESCE(bullet_patch.expire_at, bullet.expire_at),
              active_at = COALESCE(bullet_patch.active_at, bullet.active_at)
            FROM bullet_patch
          ...
          id = this._sql.get`SELECT bullet_id FROM bullet_patch WHERE id = ${patch_id}`.bullet_id;
        }
  ...
  /**
   * Revert a patch
   * @param {number} patch_id - ID of patch to revert
   * @return {number} ID of the reverted patch
   */
  patch_bullet_revert(patch_id) {
    throw new Error('Not Implemented');
    // NOTE: deal with branched versioning; probably a good time for hash-based version.
    // NOTE: might lead to development of merging different patch branch.
    // NOTE: too much for a barely functioning application.
  }
}
