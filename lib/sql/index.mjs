...
export class Database {
  ...
  patch_bullet_create_create(patch) {
    try {
      const res = this._validateModification(
        this._sql.run`
          INSERT INTO bullet_patch (content, is_active, expire_at, active_at)
          VALUES (
            ${patch.content},
  ...
  patch_bullet_create_update(bullet_id, patch) {
    try {
      const res = this._validateModification(
        this._sql.run`
          INSERT INTO bullet_patch (bullet_id, bullet_version, content, is_active, expire_at, active_at)
          VALUES (
            ${bullet_id},
    ...
    try {
      ...
      try {
        ...
        let res = this._sql.run`
          INSERT INTO bullet (id, content, is_active, expire_at, active_at, version)
          SELECT bullet_id, content, COALESCE(is_active, 1), expire_at, active_at, 1 
          FROM bullet_patch
          WHERE id = ${patch_id}
            AND status = 'P'
